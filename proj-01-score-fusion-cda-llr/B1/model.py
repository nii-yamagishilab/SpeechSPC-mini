...
class BackEnd(torch.nn.Module):
    ...
    def get_ilr(self, cm_scores, asv_scores):
        # not real ilr, just some scores without scaling
...
class BackEndLoss(torch.nn.Module):
    ...
    def forward(self, cm_logits, cm_scores, asv_logits, asv_scores, 
        ...
        # training for tar-bonafide against others
        p2_label = (label_cm * label_asv).squeeze().to(dtype=asv_logits.dtype)
        loss_p_2 = self.bce(rev_p_2, p2_label)
        
        # tarining for nontarget against spoof
        # label_cm: cm
        # label_asv: asv
        # only samples not target bonafide
        idx = p2_label < 1
        p1_label = label_cm[idx].squeeze().to(dtype=asv_logits.dtype)
        loss_p_1 = self.bce(rev_p_1[idx], p1_label)
        return loss_cm
...
class SASVBaseline(sb.core.Brain):
    ...
    def on_stage_end(self, stage, stage_loss, epoch=None):
        ...
        if stage == sb.Stage.VALID:
            ...
            rev_p_1, rev_p_2 = self.modules.backend.get_ilr(dev_cm_score, dev_asv_score)
            dev_scores = self.modules.backend.get_score(
                None, dev_cm_score, None, dev_asv_score, rev_p_1, rev_p_2)
