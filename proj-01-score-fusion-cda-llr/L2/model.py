...
class BackEnd(torch.nn.Module):
    ...
    def forward(self, asv_enro, asv_test, cm_enro, cm_test):
        ...
        # ===
        # irl
        rev_p_1, rev_p_2 = self.get_ilr(llr_tarspf, llr_tarnon)

        return cm_logits, llr_tarspf, asv_logits, llr_tarnon, rev_p_1, rev_p_2
    ...
    def _gaussian_calib_fit(
        ...
        # CM label: use tar,non versus spoofed
        index_cm_data = np.full(labels.shape, True)
        # [True: bona fide, False: spoofed]
        labels_cm = labels[index_cm_data] != spoof_label_val
    ...
    def get_ilr(self, llr_tarspf, llr_tarnon):
        """ p_1, p_2 = get_ilr(llr_tarspf, llr_tarnon)
        Return composed weight of evidence given LLRs from CM and ASV
        
        input
        -----
          llr_tarspf: tensor, (N, ), CM llrs for N data samples
          llr_tarnon: tensor, (N, ), ASV llrs for N data samples
        
        output
        ------
          rev_p_1: tensor, (N, ), to discriminate tarnon from spoof
          rev_p_2: tensor, (N, ), to discriminate tarbon from others
        """

        # a higher rev_p_2 favors tar bonafide against others
        rev_p_2 = - (- llr_tarspf - llr_tarnon) / np.sqrt(6.0)
        # a higher rev_p_1 favors tar nontarget against spoofed
        rev_p_1 = -(-llr_tarspf + llr_tarnon) / np.sqrt(2.0)        
        return rev_p_1, rev_p_2
    
    def get_score(self, cm_logits, llr_tarspf, asv_logits, llr_tarnon, 
                  rev_p_1, rev_p_2):
    ...
    def get_cm_scores(self, cm_logits, llr_tarspf, asv_logits, llr_tarnon, 
                      tmp1, tmp2):
    ...
    def get_asv_scores(self, cm_logits, llr_tarspf, asv_logits, llr_tarnon, 
                       tmp1, tmp2):
